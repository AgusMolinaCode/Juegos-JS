...
timerId = setInterval(moveBall,22)

function checkForCollisions() {
    for(let i = 0; i < blocks.length; i++) {
        if (
            (ballCurrentPosition[0] > blocks[i].bottomLeft[0] && ballCurrentPosition[0] < blocks[i].bottomRight[0]) && ((ballCurrentPosition[1] + ballDiameter) > blocks[i].bottomLeft[1] && ballCurrentPosition[1] < blocks[1].topLeft[1])) 
            {
    ...
    if (ballCurrentPosition[1] <= 0) {
        clearInterval(timerId);
        scoreDisplay.innerHTML = 'LOOOSER!!';
        document.removeEventListener('keydown',moveUser)
    }
